package com.sandy.jeecoach.util.qsorter;
...
public class TopicButtonPanel extends JPanel implements ActionListener {
    ...
    private JEEQSorterApp sorter = null ;
    private String selectedTopic = null ;

    private JTabbedPane tabPane = new JTabbedPane() ;
    
    public TopicButtonPanel( JEEQSorterApp sorter ) {
        this.sorter = sorter ;
        this.baseDir = sorter.getBaseDestDir() ;
        populateModel( phyTopicNames,  JEEQSorterApp.SUB_PHY   ) ;
        populateModel( chemTopicNames, JEEQSorterApp.SUB_CHEM  ) ;
        populateModel( mathTopicNames, JEEQSorterApp.SUB_MATHS ) ;
        setUpUI() ;
    }
    ...
    public void showTab( String subName ) {
        switch( subName ) {
            case JEEQSorterApp.SUB_PHY:
                tabPane.setSelectedIndex( 0 ) ;
                break ;
                
            case JEEQSorterApp.SUB_CHEM:
                tabPane.setSelectedIndex( 1 ) ;
                break ;
                
            case JEEQSorterApp.SUB_MATHS:
